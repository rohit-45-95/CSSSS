...
class LaunchArgs(
    ...
    private val authlibArgs: List<String> = emptyList()
) {
    fun getAllArgs(): List<String> {
        val argsList: MutableList<String> = ArrayList()

        argsList.addAll(getJavaArgs())
        argsList.addAll(authlibArgs)
        
        val jvmArgs = getMinecraftJVMArgs()
        argsList.addAll(jvmArgs)

        // Fix: Prevent double -cp bug for modern Minecraft versions
        if (!jvmArgs.contains("-cp")) {
            argsList.add("-cp")
            // Build classpath properly: if lwjgl3 is empty, don't prepend colon
            val lwjgl3CP = Tools.getLWJGL3ClassPath()
            val classPath = if (lwjgl3CP.isNotEmpty()) {
                "$lwjgl3CP:$launchClassPath"
            } else {
                launchClassPath
            }
    ...
    private fun getMinecraftJVMArgs(): Array<String> {
        ...
        // Prepare placeholder map used by version JSON JVM arguments
        val lwjgl3CP = Tools.getLWJGL3ClassPath()
        val classPath = if (lwjgl3CP.isNotEmpty()) {
            "$lwjgl3CP:$launchClassPath"
        } else {
    ...
    private fun getMinecraftClientArgs(): Array<String> {
        ...
        val isOffline = AccountUtils.isNoLoginRequired(account)
        verArgMap["auth_session"] = if (isOffline) "0" else account.accessToken
        verArgMap["auth_access_token"] = if (isOffline) "0" else account.accessToken
        ...
        verArgMap["user_type"] = if (isOffline) "legacy" else "msa"
    ...
    private fun setLauncherInfo(verArgMap: MutableMap<String, String>) {
        ...
        // FIX: The Client ID is properly mapped here so it parses directly into the JSON arguments safely
